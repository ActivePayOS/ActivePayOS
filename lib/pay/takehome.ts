...
export function computeTakeHome(i: TakeHomeInput): TakeHomeResult {
  ...
  // TSP is a % of base pay. Traditional contributions are pre-tax (reduce
  // federal/state taxable income); Roth contributions are not.
  const tspMonthly = Math.max(0, Math.min(1, i.tspPct)) * base;
  ...
  const medicareMonthly = taxableMonthly * MEDICARE_RATE;
